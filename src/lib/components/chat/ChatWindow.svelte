		...
		<div class="w-full">
			...
			>
				...
				{:else}
					<div class="flex w-full flex-1 border-none bg-transparent">
						{#if lastIsError}
							<ChatInput value="Что-то пошло не так..." disabled={true} />
						{:else}
							<ChatInput
			...
			>
				<p>
					...
					> <span class="max-sm:hidden">·</span><br class="sm:hidden" /> Созданный контент может быть неточным
					или ложным.
				</p>
				{#if messages.length}
					<button
					...
					>
						...
						<div class="max-sm:hidden">Поделиться чатом</div>
					</button>
				{/if}
			</div>
		</div>
